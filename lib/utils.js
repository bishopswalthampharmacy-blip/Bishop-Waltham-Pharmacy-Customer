...
export async function bookSlotForDay(bookingData) {
  ...
  const payload = {
    ...
    vaccinesIDs: finalVaccineNames, // Using the same field for both vaccine and consultancy names
    ...
    type: appointmentType === "consultation" ? "Consultation" : "Vaccination",
    consultancyType:
...
export async function fetchConsultancyTypes() {
  try {
    const response = await fetch(`${API_BASE_URL}/get-consultancy-types`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
export async function fetchSlotsForDay(date) {
  try {
    const response = await fetch(
      "https://7n0wver1gl.execute-api.eu-west-2.amazonaws.com/dev/fetch-slot-of-a-day",
      {
        method: "POST",
