...
export const FREE_CONSULTANCY_TYPES = [
  "Travel Clinic",
  "Ear Microsuction",
  "Weight Loss",
];

export function isFreeConsultancyType(type = "") {
  return FREE_CONSULTANCY_TYPES.includes(type);
}
    ...
    type:
      appointmentType === "consultation"
        ...
        : appointmentType === "ear-microsuction"
          ? `Ear Microsuction - ${earMicrosuctionType || "Selected Option"}`
          : "Vaccination",
    consultancyType:
      appointmentType === "consultation"
        ? consultancyType
...
export async function fetchAllBlogs() {
  try {
    const response = await fetch(`${API_CONFIG.BASE_URL}/get-all-blogs`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
    ...
    const responseData = await response.json()
    console.log("Fetch all blogs API response:", responseData)
    ...
    if (responseData.responseStatus?.code === 1001) {
      ...
      return {
        success: false,
        message: responseData.responseStatus?.message || "Failed to fetch blogs",
        blogs: [],
      }
    }
  } catch (error) {
