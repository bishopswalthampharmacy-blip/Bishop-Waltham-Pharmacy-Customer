...
export default function BookingPage({ cartItems = [] }) {
  ...
  const proceedToBooking = async (time) => {
    ...
    try {
      ...
      const bookingData = {
        ...
        cartItems,
        vaccineNames: appointmentType === 'vaccination' ? cartItems.map(item => ({
  ...
  const getCurrentStepNumber = () => {
    ...
    // Normal consultation flow (6 steps total)
    switch (currentStep) {
  ...
  return (
    <><Dialog open={!!paymentStatus} onOpenChange={() => setPaymentStatus(null)}>
      {paymentStatus && (
        <DialogContent className="sm:max-w-lg text-center bg-gradient-to-br from-white via-slate-50 to-sky-50 shadow-2xl rounded-3xl p-10 border border-slate-200">
          <motion.div
          ...
          >
            <DialogHeader>
              {paymentStatus === "success" ? (
                ...
                >
                  <CheckCircle2 className="mx-auto text-green-500 w-24 h-24 mb-6 drop-shadow-lg animate-bounce" />
                </motion.div>
              ) : (
                <XCircle className="mx-auto text-red-500 w-24 h-24 mb-6 drop-shadow-lg" />
              )}
              ...
              <DialogDescription className="text-gray-600 mt-4 text-lg leading-relaxed max-w-md mx-auto">
                {paymentStatus === "success"
            ...
            {paymentStatus === "success" && (
              <p
                className="mt-6 font-semibold text-lg px-4 py-3 rounded-xl inline-block shadow-sm"
                style={{ backgroundColor: "#037F9110", color: "#037F91" }} // 10% opacity for bg
              >
                Please arrive at least <span className="font-bold">10 minutes before</span> your booking slot.
              </p>
            )}
            <Button
              onClick={handleGoHome}
              className="bg-[#0B5C64] text-white hover:bg-[#09494e]"
              style={{ backgroundColor: "#0B5C64", color: "#fff", margin: "5%" }}
            >
              OK
            </Button>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-white">
                ...
                <div className="border border-dashed border-white rounded-lg p-6 flex items-start space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-2xl font-bold mt-1">01489892599</p>
                  </div>
                </div>
